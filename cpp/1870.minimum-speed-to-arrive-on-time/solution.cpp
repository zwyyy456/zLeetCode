// Created by zwyyy456 at 2023/06/01 11:47
...
before you can depart on the `2ⁿd` train ride at the 2 hour mark.
...
class Solution {
  public:
    bool Check(vector<int> &dist, double hour, int vel) {
        double sum = 0;
        for (int i = 0; i < dist.size() - 1; ++i) {
            sum += (dist[i] - 1) / vel + 1;
        }
        return sum + (double)(dist.back()) / vel <= hour;
    }
...
int main() {
    ...
    double hour;
    LeetCodeIO::scan(cin, hour);

    Solution *obj = new Solution();

    auto res = obj->minSpeedOnTime(dist, hour);
