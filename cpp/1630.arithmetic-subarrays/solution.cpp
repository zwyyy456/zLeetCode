// Created by zwyyy456 at 2023/03/23 09:28
...
Explanation:
In the 0th query, the subarray is [4,6,5]. This can be rearranged as [6,5,4], which is an arithmetic
sequence.
In the 1st query, the subarray is [4,6,5,9]. This cannot be rearranged as an arithmetic sequence.
In the 2ⁿd query, the subarray is [5,9,3,7]. This can be rearranged as [3,5,7,9], which is an
arithmetic sequence.
```
...
class Solution {
public:
    bool check(vector<int> &nums, vector<int> &l, vector<int> &r, int idx) {
        vector<int> vec(nums.begin() + l[idx], nums.begin() + r[idx] + 1);
        std::sort(vec.begin(), vec.end());
        for (int i = 1; i < vec.size(); i++) {
            if (vec[i] - vec[i - 1] != vec[1] - vec[0]) {
                return false;
            }
        }
        return true;
