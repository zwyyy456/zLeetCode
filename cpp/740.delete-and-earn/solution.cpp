// Created by zwyyy456 at 2023/06/02 09:50
...
class Solution {
  public:
    int deleteAndEarn(vector<int> &nums) {
        ...
        int n = mp.size();
        vector<pair<int, int>> vp(mp.size());
        int i = 0;
        for (auto &p : mp) {
            vp[i++] = p;
        }
        vector<int> dp(n + 1);
        dp[1] = vp[0].first * vp[0].second;
        for (int i = 2; i <= n; ++i) {
            // 考虑前 n 个数
            if (vp[i - 1].first - vp[i - 2].first == 1) {
                dp[i] = max(dp[i - 2] + vp[i - 1].first * vp[i - 1].second, dp[i - 1]);
            } else {
                dp[i] = dp[i - 1] + vp[i - 1].first * vp[i - 1].second;
            }
        }
        return dp[n];
    }
...
int main() {
    ...
    auto res = obj->deleteAndEarn(nums);
