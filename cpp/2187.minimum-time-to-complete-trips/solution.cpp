// Created by zwyyy456 at 2023/06/01 10:33
...
class Solution {
  public:
    bool Check(vector<int> &time, long mid, int totalTrips) {
        long sum = 0;
        for (int num : time) {
            sum += mid / num;
        }
...
int main() {
    ...
    int totalTrips;
    LeetCodeIO::scan(cin, totalTrips);

    Solution *obj = new Solution();

    auto res = obj->minimumTime(time, totalTrips);
