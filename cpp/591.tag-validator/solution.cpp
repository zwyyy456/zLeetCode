// Created by zwyyy456 at 2023/05/25 09:47
    ...
    bool isValid(string code) {
        ...
        for (int i = 0; i < code.size(); ++i) {
            ...
            if (state == 0) {
                ...
                state = 1;
            } else if (state == 1) { // 正在读取 start_tag
                if (start_tag.size() > 9) {
                    cout << "more than 9\n";
                    return false;
                }
                if (c <= 'Z' && c >= 'A') {
                    start_tag.push_back(c);
                } else if (c == '/') {
                    state = 4;
                } else if (c == '!') {
                    state = 5;
            ...
            } else if (state == 3) { // 表示正在读取内容
                if (c == '<') {
                    state = 1;
                }
            } else if (state == 4) { // 表示在读取 end_tag
                if (start_tag.size() > 9) {
                    return false;
                }
                if (c <= 'Z' && c >= 'A') {
                    ...
                    if (tags.empty() || end_tag != tags.top()) {
                        return false;
                    }
                    if (end_tag.empty()) {
                        return false;
            ...
            } else if (state == 5) { // 准备读取 CDATA
                if (c == '[') {
                    state = 6;
                } else {
                    return false;
                }
            } else if (state == 6) { // 读取 CDATA
                if (i + 6 <= code.size() && code.substr(i, 6) == "CDATA[") {
                    if (tags.empty()) {
                        return false;
                    }
                    i += 5;
            ...
            } else if (state == 7) { // 读取CDATA_CONTENT
                if (i + 3 <= code.size()) {
                    if (code.substr(i, 3) == "]]>") {
                        state = 3;
                        i += 2;
