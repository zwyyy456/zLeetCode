# [1707. Maximum XOR With an Element From Array][link] (Hard)
...
array, where `queries[i] = [xᵢ, mᵢ]`.

The answer to the `iᵗʰ` query is the maximum bitwise `XOR` value of `xᵢ` and any element of `nums`
that does not exceed `mᵢ`. In other words, the answer is `max(nums[j] XOR xᵢ)` for all `j` such that
`nums[j] <= mᵢ`. If all elements in `nums` are larger than `mᵢ`, then the answer is `-1`.
